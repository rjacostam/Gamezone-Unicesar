# TEAM.md — GameZone Unicesar

Team of exactly 3 members.

| Role | Full Name | Student Code | Module | Branch |
|------|-----------|--------------|--------|--------|
| Tech Lead — Sales & Integration | Rafael Junior Acosta Mendoza | 1005061643 | Sales (`model/Sale`, `persistence/SaleRepository`, `service/SaleService`, `ui/ConsoleMenu`, `Main`) | `feature/sale-module` |
| Developer 1 — Products | Andres Daniel Cuello Blanco | 1120243014 | Products (`model/Product`, `model/VideoGame`, `model/Console`, `persistence/ProductRepository`, `service/ProductService`) | `feature/product-module` |
| Developer 2 — Persons | Jose David Cervantes Solano | 1066872626 | Persons (`model/Person`, `model/Customer`, `model/Seller`, `persistence/PersonRepository`, `service/PersonService`) | `feature/person-module` |

## Committed activities per member
See Taller pages 7-9. Each member must do >=12 atomic commits in English (Conventional Commits) and team >=6 approved PRs.
- Lead: repo setup, branches protection, pom + packages, TEAM.md, Sale domain + total, SaleRepository, SaleService (validations/stock), UI menu + submenus, Main with DI, PR reviews, final README.
- Dev1: product branch, abstract Product + abstract getDescription, VideoGame, Console, ProductRepository, ProductService, JavaDoc, PRs.
- Dev2: person branch, abstract Person, Customer, Seller, PersonRepository, PersonService, JavaDoc, PRs.

